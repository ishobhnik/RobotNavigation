...
Refer to Appendix \ref{app:results} for further results and details regarding the experiments.\\
\begin{figure}[H]
    \includegraphics[width=\textwidth]{images/train_action_samples_1.png}    
    \label{fig:train_action_samples_1}
    \caption{Illustrates a scenario where a robot is trying to navigate towards a specific goal (Train Action Sample)}
\end{figure}
\newpage
...
\end{figure}
Figure~\ref{fig:diffusion_loss_figure} compares training and evaluation diffusion losses under random, no, and goal masking strategies.\
\begin{enumerate} \item \ref{fig:diffusion_loss}: Training loss decreases sharply around 30k–40k steps and then stabilizes, indicating effective learning.
\item \ref{fig:diffusion_eval_loss_goal_masking}, \ref{fig:diffusion_eval_loss_no_masking}, \ref{fig:diffusion_eval_loss_random_masking}: Evaluation losses are higher and more unstable, but all show an initial sharp decline. 
\end{enumerate}
\subsection*{Distance Loss and Total Loss}
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
...
\begin{itemize}
    ...
    \item \hyperref[app:vit]{\textbf{Appendix F:}} Vision Transformer
