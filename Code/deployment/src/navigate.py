...
import yaml
import time
from collect_offline_data import OfflineDataCollector
...
def main(args: argparse.Namespace):
    ...
    data_collector = OfflineDataCollector(OFFLINE_DATA_DIR, model_params)
    ...
    while not rospy.is_shutdown():
        ...
        if len(context_queue) > model_params["context_size"]:
            if model_params["model_type"] == "nomad":
                ...
                data_collector.store_context_vector(obsgoal_cond.detach().cpu().numpy())
                ...
                with torch.no_grad():
                    ...
                    for k in noise_scheduler.timesteps[:]:
                        ...
                        # Store predicted noise
                        data_collector.store_predicted_noise(noise_pred.detach().cpu().numpy())
        ...
        if reached_goal:
            ...
            # Save episode data if goal reached
            data_collector.save_episode(episode_num)
            episode_num += 1
            reached_goal = False
        rate.sleep()
